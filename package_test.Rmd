...
bin_time_irt_2pl_abs_est <- id_estimate(idealdata=bin_time_irt_2pl_abs_sim,
                               model_type=2,
                               restrict_ind_high = as.character(sort(bin_time_irt_2pl_abs_sim@simul_data$true_person,
                                                                     decreasing=T,
                                                        index=T)$ix[1]),
                              restrict_ind_low = as.character(sort(bin_time_irt_2pl_abs_sim@simul_data$true_person,
                                                                   decreasing=F,
                                                        index=T)$ix[1]),
                               fixtype='constrained',
                              id_diff = sort(bin_time_irt_2pl_abs_sim@simul_data$true_person,decreasing=T)[1] -
                             sort(bin_time_irt_2pl_abs_sim@simul_data$true_person,decreasing=F)[1],
                             time_sd=10,
                             restrict_sd = .01)
