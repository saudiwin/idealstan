...
num_person <- 20
  ...
  time_points <- rep(1:time,each=num_bills/time_points)
...
```{r irt_binary_par}
bin_irt_2pl_sim_par <- id_sim_gen(num_person=20,num_bills=200,ordinal=F,inflate=F,
                              diff_sd=1,
                              reg_discrim_sd = 1,
                              absence_discrim_sd = 1)
...
ord_irt_abs_par <- id_estimate(idealdata=ord_irt_abs_sim_par,
                              ...
                              restrict_ind_low = as.character(sort(ord_irt_abs_sim@simul_data$true_person,
                                                                   decreasing=F,
                                                        index=T)$ix[1]),
                               fixtype='prefix',id_diff =sort(ord_irt_abs_sim@simul_data$true_person,
                                                                   decreasing=T)[1] -
                                sort(ord_irt_abs_sim@simul_data$true_person,
                                  decreasing=F)[1])
...
ord_irt_grm_par <- id_estimate(idealdata=ord_irt_grm_sim_par,
                              ...
                              restrict_ind_low = as.character(sort(ord_irt_grm_sim@simul_data$true_person,
                                                                   decreasing=F,
                                                        index=T)$ix[1]),
                               fixtype='prefix',id_diff =sort(ord_irt_grm_sim@simul_data$true_person,
                                                                   decreasing=T)[1] -
                                sort(ord_irt_grm_sim@simul_data$true_person,
                                  decreasing=F)[1])
...
num_person <- 20
 ...
 person_points <- person_points_all[(2*num_per_dist+1):(num_per_dist*3)]
  ...
  combine_data_est_map <- id_estimate(combine_data,nchains=4,ncores = 20,fixtype = "prefix",restrict_ind_high = as.character(sort(ideal_pts[,1],
                                                                     decreasing=T,
                                                        index=T)$ix[1]),
                               restrict_ind_low = as.character(sort(ideal_pts[,1],
                                                                    decreasing=F, 
