...
#' @export
plot_sims <- function(sims,type='RMSE') {
  ...
  ggplot(over_params,aes(y=avg,x=reorder(param_id,avg),ymax=high,ymin=low)) + geom_pointrange() +
    theme_minimal() + xlab("Parameters") + ylab(type) + facet_wrap(~param_type,scales = "free_x",
                                                                   ncol=3)
