...
#'    ordinal outcomes and 2 for binary outcomes. Optionally, you can also just pass in a \code{\link[pscl]{rollcall}} object.
...
#' @param varying_group_id A vector equal to the length of the number of votes (i.e. the concatenated score_data matrix)
#' with IDs that assign each person/legislator to a group. Entering the group IDs manually in this 
#' fashion allows you to use time-varying group IDs instead of the static group IDs from the 
#' person_data.
#' @param item_cov A matrix with rows equal to the number of items (bills) with covariates for hierarchical modeling of item (bill) discrimination parameters in the non-inflated model, i.e., non-missing model
#' @param item_cov_miss A matrix with rows equal to the number of items (bills) with covariates for hierarchical modeling of item (bill) discrimination parameters in the inflated model (missing data model)
...
#' @param time An optional integer, Date or date-time (POSIXlt or POSIXct) vector of length equal to the number of columns in the response matrix (i.e., the number of items or bills)
...
#'  ideal points are allowed to vary over time. You can also pass in the string 'separate' if the score matrix column names are coded as 
#'  itemname_time, and the time values will be extracted as whatever numeric values come after the underscore in the 
#'  column names.
...
# Exogenous Missing Covariates --------------------------------------------
  ...
  outobj <- new('idealdata',
      ...
      miss_val=miss_val,
      person_vals=person_vals,
      group_vals=group_vals,
      group_varying=!is.null(varying_group_id))
...
id_estimate <- function(idealdata=NULL,model_type=2,use_subset=FALSE,sample_it=FALSE,
  ...
  if(use_groups==T) {
    if(!idealdata@group_varying) {
      num_legis <- idealdata@group_vals
      legispoints <- rep(num_legis,times=num_bills)
    } else {
      legispoints <- idealdata@group_vals
    }
  } else {
    num_legis <- idealdata@person_vals
    legispoints <- rep(num_legis,times=num_bills)
  }

  num_bills <- ncol(idealdata@score_matrix)
  billpoints <- rep(1:num_bills,each=nrow(idealdata@score_matrix))
