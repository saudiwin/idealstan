...
`Parameter Name` is the name of the parameter in the underlying Stan code. If you want to peruse the Stan object, which can be accessed as follows. The name of the parameters for ideal points in the Stan model is `L_full` (as seen in the summary from above).
...
If we know the name of the Stan parameter, we can look at the trace plot to see how the MCMC sampling went. A good trace plot shows a bouncy line that is stable around an average value. For more info, see the [Stan documentation](http://mc-stan.org/users/documentation/case-studies/rstan_workflow.html).
