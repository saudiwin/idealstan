...
The function `id_sim_gen()` allows you to simulate data from any of the fourteen models currently implemented in `idealstan` (see previous list). For example, here we sample data from a ordinal graded response model:
...
To demonstrate how the package functions empirically, I include in the package the complete voting record of the 114th Senate from the website (www.voteview.com). We can convert this data, which is provided in a long data frame, to an `idealdata` object suitable for estimation by using the `id_make` function. Because the intention is to fit a binary outcome model (yes or no votes on bills), I specify `ordinal=FALSE` to exclude any other outcomes (such as abstentions). 
...
hist(senate114$cast_code)

```

First, you can see that the data are in long form because there is one row for every vote cast by a legislator, in this case Senator Sessions. The outcome variable is `cast_code`. We also know the dates of the individual items (rollcalls), which we will use in the time-varying ideal point vignette to model time-varying ideal points.

The plot shows that there are roughly twice as many yes votes (value of 2) versus no votes (value of 1), with a small minority of absences (value of 3). In this case, absences can be counted as missing data.
...
We also need to be sure to code the outcome so that it corresponds to our binary outcome (yes/no) model with missing-data marked as legislator absences. First, we want to specify that the value of 9 in the data should be marked as missing by passing this value to the `miss_val` option as this value implies that the legislator chose to miss the vote. We also want to specify the `low_val` and `high_val` options to signify which values correspond to `yes` and `no` votes (i.e, the binary 0/1 outcome). In this dataset, a value of 6 actually means a no vote while a 1 signifies a yes vote. Finally we set 0 as `exclude_level` because this value signifies that the legislator was not a member of the legislature during this time, which we can consider missing data that does not need to be included in the model. If the legislator did not have a choice in the matter, then there is no reason to model it using the hurdle function as there is no way the legislator could have shown up to vote, and hence is ignorable.
...
sen_est <- id_estimate(senate_data,
            ...
            seed=84520)
id_plot_legis(sen_est,person_ci_alpha=0.7) +
...
id_plot_legis(sen_est,person_ci_alpha=0.1,item_plot='94') +
  scale_color_manual(values=c(D='blue',R='red',Indep='green')) +
...
`Parameter Name` is the name of the parameter in the underlying Stan code. If you want to peruse the Stan object, which can be accessed as follows:
...
```{r cov_plot_small}
id_plot_cov(sen_est_cov,cov_type='person_cov') + xlim(c(-0.1,0.1)) +
  ggtitle('Interaction of Party and Age on Ideal Point Scores in 114th Senate')
```


Because these covariates are on the ideal point scale, the meaning of the scale in terms of party ideology must be kept in mind in order to interpret the plot. First, because we only have estimates for the "I" and "R" values of `party_code`, we know that the Democrats "D" are being used as the reference value. As a consequence, the coefficient for `age` is equal to the effect of age for Democrats (i.e., when the person is neither Republican or Independent). What is interesting is that the effect of `age` is similar for both Republicans and Democrats. Because both of these groups are on *opposite* sides of the ideal point spectrum, the substantive interpretation is that older Republicans tend to be *more* conservative/negative (see plot above), while older Democrats tend to be more moderate than younger Democrats.

Of course, because this model was fit with variational inference (`use_vb=T`), the results are an approximation. In this case given the size of the data it would be a good idea to run full Bayesian inference. 
