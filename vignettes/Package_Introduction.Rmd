...
Because this dataset is relatively large, we will use the `use_vb` option to use Stan's variational Bayesian inference. This version of the sampler is less accurate and tends to underestimate uncertainty, but it runs much, much faster. I constrain a conservative senator (Ben Sasse) to be positive, and a liberal senator, Bernie Sanders, to be negative in order to identify the polarity in the model. I have to pass in the names of the legislators as they exist in the IDs present in the data. The `seed` argument is passed along to the underlying Stan engine.
...
```{r item_plot}
id_plot_legis(sen_est,person_ci_alpha=0.1,item_plot='Vote 94') +
...
id_plot(sen_est,person_ci_alpha=0.1,item_plot='Vote 94',
        item_plot_type='inflated') + scale_colour_brewer(type='qual') +
  scale_color_manual(values=c(D='blue',R='red',Indep='green'))
```

This absence midpoint shows that absences are very uninformative of ideal points for this particular bill (as is common in the Senate where absences are very rare).