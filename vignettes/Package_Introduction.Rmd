...
Ideal points are not identified without further information about the latent scale, especially its direction: should conservative ideal points be listed as positive or negative? To identify the latent scale, I constrain a conservative senator (Ben Sasse) to be positive, and a liberal senator, Bernie Sanders, to be negative in order to identify the polarity in the model. I have to pass in the names of the legislators as they exist in the IDs present in the data (column `bionames`). Rather than specify the exact values to pin the legislators to, I set `fixtype` to `'vb_partial'` so that the function will first fit an un-identified model, and then use those estimates to determine what would be good values to pin these legislators to. Using `'vb_partial'` is the recommended way to identify models while using prior information about the direction of the latent scale as it avoids having to fix these ideal points to specific values.
...
sen_est <- id_estimate(senate_data,
                model_type = 1,
                 use_vb = F,
                fixtype='vb_partial',
                 restrict_ind_high = "WYDEN, Ronald Lee",
                 restrict_ind_low="WICKER, Roger F.",
            seed=84520)
...
We can also extract the individual posterior iterations to do addditional calculations that average over posterior uncertainty by changing the `aggregate` option:
