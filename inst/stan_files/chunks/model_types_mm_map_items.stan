...
// skip all for pad_id=0
  ...
  for(n in end2:start2) {
    ...
    if(mm[n]==1) {
      ...
      if(T==1) {
        log_prob += bernoulli_logit_lpmf(Y_int[n]|(sigma_reg_full[s] + srx_pred[n,]*sigma_reg_x) *  (L_full[ll[n]] + legis_pred[n,]*legis_x) - B_int_free[s]);
      } else {
        log_prob += bernoulli_logit_lpmf(Y_int[n]|(sigma_reg_full[s] + srx_pred[n,]*sigma_reg_x) *  (L_tp1[time[n],ll[n]] + legis_pred[n,]*legis_x) - B_int_free[s]);
      }
    } else if(mm[n]==2) {
      //2 PL inflation
  ...
  if(T==1) {
    ...
    if(order_cats_rat[n]==3) {
      log_prob += ordered_logistic_lpmf(Y_int[n]|(sigma_reg_full[s] + srx_pred[n,]*sigma_reg_x) *  (L_full[ll[n]] + legis_pred[n,]*legis_x) - B_int_free[s],
              steps_votes3);
    ...
    if(order_cats_rat[n]==3) {
      log_prob += ordered_logistic_lpmf(Y_int[n]|(sigma_reg_full[s] + srx_pred[n,]*sigma_reg_x) *  (L_tp1[time[n],ll[n]] + legis_pred[n,]*legis_x) - B_int_free[s],
              steps_votes3);
    } else if(order_cats_rat[n]==4) {
      log_prob += ordered_logistic_lpmf(Y_int[n]|(sigma_reg_full[s] + srx_pred[n,]*sigma_reg_x) *  (L_tp1[time[n],ll[n]] + legis_pred[n,]*legis_x) - B_int_free[s],
              steps_votes4);
...
if(T==1) {
  ...
  //poisson no inflation
      ...
      if(T==1) {
        log_prob += poisson_log_lpmf(Y_int[n]|(sigma_reg_full[s] + srx_pred[n,]*sigma_reg_x) *  (L_full[ll[n]] + legis_pred[n,]*legis_x) - B_int_free[s]);
      } else {
        log_prob += poisson_log_lpmf(Y_int[n]|(sigma_reg_full[s] + srx_pred[n,]*sigma_reg_x) *  (L_tp1[time[n],ll[n]] + legis_pred[n,]*legis_x) - B_int_free[s]);
      }
