...
if(T<50) {
  ...
  if(restrict_var==1) {
    ...
    if(s==1) {
      ...
      log_prob += normal_lpdf(L_tp1_var[2:T,s]|L_tp1_var[1:(T-1),s],time_sd]));
      
      
    } else {
      
      log_prob += normal_lpdf(L_tp1_var[2:T,s]|L_tp1_var[1:(T-1),s],time_var_free[s-1]));
      
    }
    
  } else {
    
    log_prob += normal_lpdf(L_tp1_var[2:T,s]|L_tp1_var[1:(T-1),s],time_var_free[s]));
