...
if(model_type==1) {
  //2 PL no inflation
      ...
      if(T==1) {
        ...
        for(n in 1:N) {
          if(time[n]==1) {
            pi1[n] = sigma_reg_full[bb[n]] *  L_full[ll[n]] - B_int_full[bb[n]];
          } else {
            pi1[n] = sigma_reg_full[bb[n]] *  L_tp1[time[n]-1,ll[n]] - B_int_full[bb[n]];
          }
        }
      }
  ...
  //2 PL inflation
      ...
      if(T==1) {
        ...
        for(n in 1:N) {
          if(time[n]==1) {
            pi1[n] = sigma_reg_full[bb[n]] *  L_full[ll[n]] - B_int_full[bb[n]];
            pi2[n] = sigma_abs_full[bb[n]] * L_full[ll[n]] - 
                    A_int_full[bb[n]] + exog_data[n]*exog_param; 
          } else {
            pi1[n] = sigma_reg_full[bb[n]] *  L_tp1[time[n]-1,ll[n]] - B_int_full[bb[n]];
            pi2[n] = sigma_abs_full[bb[n]] * L_tp1[time[n]-1,ll[n]] - 
                    A_int_full[bb[n]] + exog_data[n]*exog_param; 
          }
